...
대표 운영자용 Admin Copilot deterministic MVP가 `feat/admin-copilot-prd` branch에 구현되어 있습니다. Supabase 운영 데이터를 permission-scoped service에서 집계하고, 네 가지 명시적 규칙으로 evidence-backed task를 생성하여 workspace home에 추천 briefing으로 표시합니다. 출석 위험 대상은 표준 group-derived 참여자 projection과 일치하며, owner admin이 관련 참여자 현황 화면으로 이동할 수 있습니다. pending material에는 owner-only ReviewMaterial 승인 UI가 연결되어 있지만 실제 인증 runtime/browser 흐름은 아직 검증하지 않았습니다. AI provider와 자연어 후속 질문은 아직 구현하지 않았습니다. 현재 branch에 연결된 Pull Request는 없습니다.
...
Developer QA와 ReviewMaterial Task 1 DB foundation, Task 2 pure action contract helper, Task 3 proposal service, Task 4 human decision·execution service, Task 5 Admin Copilot task context 연결, Task 6 owner-only 승인 UI 구현은 완료되었습니다. 다음 작업은 `docs/ontology-action-implementation-plan.md`의 Task 7에 따라 local Supabase/Auth/browser E2E 검증을 추가하는 것입니다. pending 자료 proposal 생성·거절·stale invalidation·신규 proposal 승인·중복 승인·signal 해소와 기존 수동 review, 비 owner 차단을 실제 흐름에서 확인해야 합니다.
...
- `ReviewMaterial` Task 1 DB foundation부터 Task 6 승인 UI까지 compile/focused test는 통과했지만, 인증된 owner가 실제 server action을 호출하는 Supabase service/integration/browser runtime 검증, application-level optimistic stale-state mapping, signal-resolution integration test는 아직 없습니다.
