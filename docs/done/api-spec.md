...
type CourseFeedbackCategory = 'suggestion' | 'praise' | 'other';
type CourseFeedbackStatus = 'new' | 'reviewed';
```
...
공개 수업 피드백은 비로그인 방문자가 특정 공개 수업에 남기는 의견이다. 피드백은 공개 페이지에 노출하지 않고 워크스페이스 내부 의견 수렴 페이지에서만 확인한다.

### 19.1 공개 피드백 제출

Action: `createCourseFeedback(courseId, input)`
...
- 인증이 필요 없다.
- `courses.public_visibility = 'public'`인 수업에만 제출할 수 있다.
- `category`는 `suggestion`, `praise`, `other` 중 하나다.
- 화면 라벨은 각각 `제안`, `좋았던 점`, `기타`다.
- `authorName`, `authorPhone`은 선택 입력이다.
- `authorPhone`이 있으면 `authorName`도 필요하다.
- 전화번호는 하이픈을 허용한다.
- `privacyConsent`는 반드시 `true`여야 하며 저장 시 `privacy_consent_at`을 남긴다.
- 저장 시 `course_name_snapshot`을 함께 저장한다.
- 생성 활동 로그에는 전화번호와 전체 본문을 넣지 않는다.

### 19.2 내부 피드백 목록

Query: `getCourseFeedbacks`

```ts
type CourseFeedbackListItem = {
  id: UUID;
  workspaceId: UUID;
  courseId: UUID;
  courseName: string;
...
- owner_admin은 워크스페이스 전체 피드백을 볼 수 있다.
- group_admin은 접근 가능한 그룹과 연결된 수업의 피드백만 볼 수 있다.
- instructor는 접근할 수 없다.
- 목록은 `created_at desc` 최신순이며 1차 구현은 최근 100건만 반환한다.
- 수업 필터 옵션은 접근 가능한 결과 중 피드백이 있는 수업만 포함한다.

### 19.3 피드백 확인 상태 변경

Action: `updateCourseFeedbackStatus`
...
- owner_admin은 전체 피드백 상태를 변경할 수 있다.
- group_admin은 접근 가능한 수업의 피드백 상태만 변경할 수 있다.
- instructor는 변경할 수 없다.

### 19.4 피드백 삭제

Action: `deleteCourseFeedback`

권한:

- owner_admin만 삭제할 수 있다.
- 삭제는 물리 삭제다.
- 1차 구현에서는 삭제 활동 로그를 남기지 않는다.
